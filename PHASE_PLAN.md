...
- **Delayed settlement (maturity hold) — ✅ DONE.** A plan may set `settlement: maturity` (default `on_confirmation` = unchanged). A contract then snapshots a `settle_at` date (from the plan's harvest/needed-by date) onto `transactions.settle_at`; the buyer's confirm-to-release and a seller-initiated `releaseEscrow` are **blocked until `settle_at`**, so the producer can't be paid before delivery. The manual dispute path still works before maturity (buyer protection). After maturity, the normal rating flow releases (or `−1` freezes). Frontend: a `Settlement` select on the plan forms; the contract shows "🔒 Settles at maturity: date"; the profile hides the confirm button until matured. Verified end-to-end (blocked before, releases after, `on_confirmation` unchanged). _Milestone/progress-payment tranches deferred as a future enhancement._
