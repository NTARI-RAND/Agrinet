...
- **Escrow gate — ✅ DONE.** The consumer's (buyer's) LBTAS rating is the release trigger: `transactionService._settleEscrow` (shared, runs inside the caller's DB transaction) transitions `paid→completed` and credits the seller. The buyer's rating **auto-releases** held funds (`rateTransaction`, `source: lbtas_gate`); a seller-initiated `releaseEscrow` is now **gated** (`buyer_rated` required) and is a fallback. The producer's post-hoc rating moves no money. Verified end-to-end (release blocked pre-rating; buyer `+4` → seller credited; double-release rejected). Frontend: buyer's "Confirmar e avaliar" + a "held payment released" note in `RatePrompt`; seller's "Liberar" only shows once the buyer has rated.
